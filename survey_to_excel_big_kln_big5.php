...
$ws->write ( $row, 5, $lowTable ['total'] ['pslNo'], $tlcbf );
...
// $ws->write($row,13,$lowTable['total']['recordTotal']==0?'0.00':sprintf("%01.2f",$lowTable['total']['headWayTotal']/$lowTable['total']['recordTotal']),$rtlcbf);
...
$ws->write ( $row, 5, '載客量', $ctf );
$ws->setMerge ( $row, 6, $row, 12 );
$ws->write ( $row, 6, '乘客數目', $ctf );
$ws->write ( $row, 13, '平均', $ctrf );
$row = $row + 1;
...
for($i = 0; $i < $topTableRows; $i ++) {
	...
	$ws->write ( $row, 5, $topTable [$i] ['pslNoCount'], $cf );
	$ws->write ( $row, 6, $topTable [$i] ['onArrivalCount'], $cf );
	$ws->write ( $row, 7, $topTable [$i] ["onArrivalCountPercent"], $cf );
	$ws->write ( $row, 8, $topTable [$i] ['setDownCount'], $cf );
	$ws->write ( $row, 9, $topTable [$i] ['pickupCount'], $cf );
	$ws->write ( $row, 10, $topTable [$i] ['onDeptCount'], $cf );
	$ws->write ( $row, 11, $topTable [$i] ["onDeptCountPercent"], $cf );
	$ws->write ( $row, 12, $tempBehind, $cf );
	$ws->write ( $row, 13, $topTable [$i] ['headWayCount'], $crf );
	// $ws->write($row,13,'--',$crf);
}
// 设置顶部表格底部 1b
...
$ws->write ( $row, 5, $topTable ['total'] ['pslNoCount'], $tlcbf );
$ws->write ( $row, 6, $topTable ['total'] ['onArrivalCount'], $tlcbf );
$ws->write ( $row, 7, $topTable ['total'] ["onArrivalCountTotalPercent"], $tlcbf );
$ws->write ( $row, 8, $topTable ['total'] ['setDownCount'], $tlcbf );
$ws->write ( $row, 9, $topTable ['total'] ['pickupCount'], $tlcbf );
$ws->write ( $row, 10, $topTable ['total'] ['onDeptCount'], $tlcbf );
$ws->write ( $row, 11, $topTable ['total'] ["onDeptCountTotalPercent"], $tlcbf );
$ws->write ( $row, 12, $leftBehindCount, $tlcbf );
$ws->write ( $row, 13, $topTable ['total'] ["headWayCountTotal"], $rtlcbf );
