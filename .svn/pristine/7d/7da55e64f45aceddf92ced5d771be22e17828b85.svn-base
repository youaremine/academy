...
$sheet->getColumnDimension ( 'L' )->setWidth ( 20.6 );
...
$sheet->setCellValue ( "T{$row}", "MiniBus Terminal" );
...
$sheet->setCellValue ( "A{$row}", "(2) 如小巴在中途站「飛站」" );
...
$sheet->setCellValue ( "A{$row}", "　（即車站有乘客候車，而小巴並非因滿座而不停站），請打「＊」" );
...
$sheet->setCellValue ( "A{$row}", "Registration Plate No.\n\n車牌" );
$sheet->setCellValue ( "B{$row}", "PSL" );
...
$sheet->setCellValue ( "L{$row}", "Registration Plate No.\n\n車牌" );
$sheet->setCellValue ( "M{$row}", "PSL" );
