...
$fileName = date("Y-m",strtotime($surDateStart)) . '-SZ-Part-Time-Salary.xls';
...
$sheet->setCellValue("D{$row}","Chinese name");
...
$sheet->setCellValueByColumnAndRow(($colEnd + 1),$row,'Hour Total');
$sheet->setCellValueByColumnAndRow(($colEnd + 2),$row,'Total');
$sheet->setCellValueByColumnAndRow(($colEnd + 3),$row,'Leader');
$sheet->setCellValueByColumnAndRow(($colEnd + 4),$row,'Remark');
...
foreach($salarys as $k => $v){
	if($i > 0 && $vl['surveyorCode'] != $v['surveyorCode']){
		...
		$sheet->setCellValueByColumnAndRow(($colStart + 4),$row,round($hourTotalAll,$conf['decimal']['precision']));
		$sheet->setCellValueByColumnAndRow(($colStart + 5),$row,round($salaryTotalAll,$conf['decimal']['precision']));
		$sheet->setCellValueByColumnAndRow(($colStart + 6),$row,$vl['IsSupervisor']);
		$sheet->setCellValueByColumnAndRow(($colStart + 7),$row,"SALARY " . date("Ym",strtotime($surDateStart)));
