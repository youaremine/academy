...
for ($i = 1; $i <= $maxWeekNo; $i++) {
    ...
    $row = 8;
    $sheet->getStyle("A{$row}:L{$row}")->applyFromArray($bStyle);

    $row = 9;
    // $sheet->getStyle('A'.$row)->applyFromArray($ltStyle);
    $sheet->getStyle("A{$row}:K{$row}")->applyFromArray($ltThinStyle, false);
    $sheet->getStyle('L' . $row)->applyFromArray($ltrThinStyle);
    $sheet->getRowDimension($row)->setRowHeight(33);
    $sheet->getStyle("A{$row}:L{$row}")->getAlignment()->setWrapText(true);
    $sheet->getStyle("A{$row}:L{$row}")->getFont()->setBold(true);
    $sheet->getStyle("A{$row}:L{$row}")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

    $sheet->getStyle("A{$row}:L{$row}")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
    $sheet->getStyle("A{$row}:L{$row}")->getFill()->getStartColor()->setARGB('FFFFFF00');
    ...
    for ($j = 0; $j < $rsNo; $j++) {
        ...
        if ($j == 0) {
            ...
            if (trim($mse->surveyType) == trim($msel->surveyType)
                ...
                if (empty ($mstmp->dueDate)) $mstmp->dueDate = "-";
                ...
                $sheet->setCellValue("K{$row}", $mstmp->estimatedManHour);
                $sheet->setCellValue("L{$row}", $mstmp->onBoardCostHour);
    ...
    $sheet->getStyle("J{$row}:K{$row}")->applyFromArray($ltThinStyle, false);
    $sheet->getStyle("L{$row}")->applyFromArray($ltrThinStyle);
    ...
    $sheet->getStyle("K{$row}:L{$row}")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

    $sheet->setCellValue("J{$row}", "Total Manhour Used in this Week");
    $sheet->setCellValue("K{$row}", $estimatedManHourSummary);
    $sheet->setCellValue("L{$row}", $onBoardCostSummary);

    $row = $row + 1;
    $sheet->getStyle("A{$row}:l{$row}")->applyFromArray($tThinStyle, false);
