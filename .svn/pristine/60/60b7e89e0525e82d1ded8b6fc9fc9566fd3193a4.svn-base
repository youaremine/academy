...
$row = 0;
$row = $row + 1;
$ws->setMerge ( $row, 0, $row, 11 ); // 合并单元格
...
$ws->write ( $row, 1, '載客量', $ctf );
$ws->write ( $row, 2, '到達', $ctf );
$ws->write ( $row, 3, '離開', $ctf );
...
for($i = 0; $i < $lowTableRows; $i ++) {
	...
	$ws->write ( $row, 1, $lowTable [$i] ['pslNo'], $cf );
	$ws->write ( $row, 2, $lowTable [$i] ['arrivalTime'], $cf );
...
$row = $row + 1;
$ws->write ( $row, 0, '總數', $ltlcbf );
$ws->write ( $row, 1, $lowTable ['total'] ['pslNo'], $tlcbf );
