...
// var_dump($floders);exit();

// 檢查服務器上還未完成有收到文件時間的job no.
$apiUrl = "http://www.ozzotec.com/{$project}/api/rawdata.php?q=getNoReciveRawData";
...
foreach ( $data as $v ) {
	if (empty ( $floders [$v->districtName] [$v->jobNo] ))
		continue;
