...
$t->set_file("HdIndex", "salary.html");
...
{
	$currMonth = $_GET["ddlMonth"];
}
else
{
	$currMonth = date("Y-m", mktime(0, 0, 0, date("m") - 1, date("d"), date("Y"))); // 得到前一个月
}
$firstMonthDay = $currMonth . "-01";
$currMonthShort = date("m",strtotime($firstMonthDay));
$tmpCurrYear = date("Y",strtotime($firstMonthDay));
$privMonthTime = mktime(0,0,0,$currMonthShort-1,1,$tmpCurrYear);
$privMonth = date("Y-m",$privMonthTime);
$privMonthShort = date("m",$privMonthTime);
$nextMonthTime = mktime(0,0,0,$currMonthShort+1,1,$tmpCurrYear);
$nextMonth = date("Y-m",$nextMonthTime);
$nextMonthShort = date("m",$nextMonthTime);
$t->set_var(array(
		'privMonth'=>$privMonth,
		'privMonthShort'=>$privMonthShort,
		'currMonth'=>$currMonth,
...
for($i = 0; $i < $rsNum; $i++)
{
	...
	$t->set_var(array(
			...
			"onBoardCostFare" => $ms->onBoardCostFare?$ms->onBoardCostFare:'0',
			...
			"taTotal" => $ms->taTotal?$ms->taTotal:'0',
