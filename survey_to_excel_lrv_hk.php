<?php
/*
 * Header: Create: 2012-03-19 Auther: Xiaoqiang.Wu<jamblues@gmail.com>
 */
...
$sheet->getStyle ( "B{$row}:N{$row}" )->applyFromArray ( $tStyle, false );
$sheet->getStyle ( "O{$row}" )->applyFromArray ( $trStyle, false );
...
$sheet->setCellValue ( "O{$row}", 'Display Board' );

$row = $row + 1;
$sheet->mergeCells ( "G{$row}:H{$row}" );
$sheet->mergeCells ( "K{$row}:L{$row}" );
$sheet->getStyle ( "A{$row}" )->applyFromArray ( $lStyle, false );
$sheet->getStyle ( "O{$row}" )->applyFromArray ( $rStyle, false );
...
$sheet->setCellValue ( "O{$row}", '(Non-function)' );

$row = $row + 1;
$sheet->getStyle ( "A{$row}" )->applyFromArray ( $lbStyle, false );
$sheet->getStyle ( "B{$row}:N{$row}" )->applyFromArray ( $bStyle, false );
$sheet->getStyle ( "O{$row}" )->applyFromArray ( $rbStyle, false );
...
for($i = 0; $i < $lowTableRows; $i ++) {
	...
	$sheet->setCellValue ( "O{$row}", $lowTable [$i] ['displayBoard'] ? $lowTable [$i] ['displayBoard'] : '--' );
}
// 底部表格统计 1a
...
$sheet->getStyle ( "B{$row}:N{$row}" )->applyFromArray ( $tbStyle, false );
$sheet->getStyle ( "O{$row}" )->applyFromArray ( $trbStyle, false );
...
// 统计表格:全部设置为居中
$sheet->getStyle ( "A{$detailRow}:O{$row}" )->getAlignment ()->setHorizontal ( PHPExcel_Style_Alignment::HORIZONTAL_CENTER );
...
// 统计表格:全部设置为居中
$sheet->getStyle ( "A{$totalRow}:N{$row}" )->getAlignment ()->setHorizontal ( PHPExcel_Style_Alignment::HORIZONTAL_CENTER );
...
$fleetNoCountList = array();
$fleetNoList = array ();
$fleetNoVehCountList = array();
$fleetNoVehList = array ();
$lowTableRows = count ( $lowTable ) - 1;
for($i = 0; $i < $lowTableRows; $i ++) {
	if ($lowTable [$i] ['fleetNo'] == "" || $lowTable [$i] ['fleetNo'] == "Missing")
		continue;
    $tmpFleetNo = trim ( $lowTable [$i] ['fleetNo'] );
    $fleetNoCountList[$tmpFleetNo] += 1;
	$fleetNoList [] = $tmpFleetNo;
    if(strpos($tmpFleetNo,'/') === false){
        $fleetNoVehCountList[$tmpFleetNo] += 1;
        $fleetNoVehList [] = $tmpFleetNo;
    }else{
        $tmpFleetNoArr = explode('/',$tmpFleetNo);
        foreach($tmpFleetNoArr as $v){
            $fleetNoVehCountList[$v] += 1;
            $fleetNoVehList [] = $v;
        }
    }
}
...
if ($fleetNoListRows > 1)
	sort ( $fleetNoList );

$fleetNoVehList = UniqueArray ( $fleetNoVehList );
$fleetNoVehListRows = count ( $fleetNoVehList );
if ($fleetNoVehListRows > 1)
    sort ( $fleetNoVehList );
...
for($i = 0; $i < $fleetNoListRows; $i ++) {
    $tmpFleetNo = $fleetNoList [$i];
	$col = chr ( $m + 65 );
	$sheet->setCellValue ( "{$col}{$row}", $tmpFleetNo );
    $m++;
    $col = chr ( $m + 65 );
    $sheet->getStyle("{$col}{$row}")->getNumberFormat()->setFormatCode("(0)");
	$sheet->setCellValue ( "{$col}{$row}", $fleetNoCountList[$tmpFleetNo] );
	$m ++;
	if ($m % 14 == 0) {
		$row = $row + 1;
		$m = 0;
...
// 中间车辆部分:Registration Number (vehicles of LRVs)
$row = $row + 2;
$sheet->setCellValue ( "A{$row}", 'Registration Number (vehicles of LRVs)' );
$sheet->getStyle ( "A{$row}" )->getFont ()->setUnderline ( PHPExcel_Style_Font::UNDERLINE_SINGLE );

$row = $row + 2;
$m = 0;
for($i = 0; $i < $fleetNoVehListRows; $i ++) {
    $tmpFleetNo = $fleetNoVehList [$i];
    $col = chr ( $m + 65 );
    $sheet->setCellValue ( "{$col}{$row}", $tmpFleetNo );
    $m++;
    $col = chr ( $m + 65 );
    $sheet->getStyle("{$col}{$row}")->getNumberFormat()->setFormatCode("(0)");
    $sheet->setCellValue ( "{$col}{$row}", $fleetNoVehCountList[$tmpFleetNo] );
    $m ++;
    if ($m % 14 == 0) {
        $row = $row + 1;
        $m = 0;
...
$sheet->getStyle ( "A{$registrationRow}:N{$row}" )->getAlignment ()->setHorizontal ( PHPExcel_Style_Alignment::HORIZONTAL_LEFT );
