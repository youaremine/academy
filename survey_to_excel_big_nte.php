...
$sheet->setCellValue("I{$row}",'Ozzo Ref No. :');
$sheet->setCellValue("K{$row}",$sp->refNo);
...
// 统计表格
$row = $row + 1;
$sheet->setCellValue("M{$row}",'Table 1');
...
$schFre = $topTable['total']['busTimeCount'] == 0?0:round($surTimeDiff / $topTable['total']['busTimeCount'],0);
$obsFre = $topTable['total']['departureTimeCount'] == 0?0:round($surTimeDiff / $topTable['total']['departureTimeCount'],0);
$fleetNoList = array ();
...
$sheet->getStyle("M{$row}:N{$row}")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$sheet->setCellValue("A{$row}",'S of S Date :');
$sheet->setCellValue("C{$row}",$sofsDate);
$sheet->setCellValue("M{$row}",'Maximum:');
$sheet->setCellValue("N{$row}",$topTable['total']["headWayCountMaximum"]);
...
$sheet->getStyle("M{$row}:N{$row}")->getFont()->setBold(true);
$sheet->getStyle("M{$row}:N{$row}")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$sheet->setCellValue("M{$row}",'Minimum:');
$sheet->setCellValue("N{$row}",$topTable['total']["headWayCountMinimum"]);
...
for($i = 0;$i < $fleetNoListRows;$i ++){
	...
	$col = chr($m + 65);
	$sheet->setCellValue("{$col}{$row}",$fleetNo);
	$m ++;
	$col = chr($m + 65);
	$sheet->setCellValue("{$col}{$row}","(".$fleetNoCountList[$fleetNo].")");
	$m ++;
	if($m % 14 == 0){
		$row = $row + 1;
		$m = 0;
...
$sheet->setCellValue("C{$row}",'SOS');
$sheet->setCellValue("D{$row}",'Obs.');
$sheet->setCellValue("E{$row}",'Diff.(min)');
$sheet->setCellValue("G{$row}",'No.');
$sheet->setCellValue("H{$row}",'(%)');
$sheet->setCellValue("I{$row}",'Down');
$sheet->setCellValue("J{$row}",'Up');
...
for($i = 0;$i < $lowTableRows;$i ++){
	...
	$sheet->setCellValue("E{$row}",$lowTable[$i]['diffMinShow']);
	...
	$sheet->setCellValue("N{$row}",$lowTable[$i]['headWay']);
}
// 底部表格统计 1a
...
$sheet->setCellValue("E{$row}",$lowTable['total']['diffMinTotal']);
...
// 底部备注说明
$row = $row + 2;
$sheet->setCellValue("A{$row}",'Note: (1) Scheduled departure time from Terminus');
$row = $row + 1;
$sheet->setCellValue("A{$row}",'Note: (2) Estimated arrival time at mid-stop');
$row = $row + 1;
$sheet->setCellValue("A{$row}",'Note: (3) Observed arrival time at mid-stop');
$row = $row + 1;
$sheet->setCellValue("A{$row}",'Note: (4) Observed departure time from mid-stop');
$row = $row + 1;
$sheet->setCellValue("A{$row}",'Note: (5) "--" represents "Out of Service"');
