...
foreach($rs as $k=>$v){
    ...
    $tmp_arr = array();
    $tmp_arr['i'] = $i;
    $tmp_arr['busy_time'] = $busy_time;
    $tmp_arr['type'] = $type;
    $tmp_arr['mscId'] = $v->mscId;
    $tmp_arr['weekNo'] = $v->weekNo;
    $tmp_arr['jobNo'] = $v->jobNo;
    $tmp_arr['jobNoNew'] = $v->jobNoNew;
    $tmp_arr['enjobNoNew'] = base64_encode($conf['auth']['slat'].$v->jobNoNew);
    $tmp_arr['plannedSurveyDate'] = $v->plannedSurveyDate;
    $tmp_arr['surveyTimeHours'] = $v->surveyTimeHours;
    $tmp_arr['surveyType'] = $v->surveyType;
    $tmp_arr['surveyLocation'] = $v->surveyLocation;
    $tmp_arr['surveyLocationCn'] = $v->surveyLocationCn?$v->surveyLocationCn:$v->surveyLocation;
    $tmp_arr['routeItems'] = $v->routeItems;
    $tmp_arr['estimatedManHour'] = $v->estimatedManHour;
    $tmp_arr['remainTimeHtml'] = $remainTimeHtml;

    if(!array_key_exists($v->jobNo,$res[$v->plannedSurveyDate])){
        $res[$v->plannedSurveyDate][$v->jobNo] = array();
    }

    $res[$v->plannedSurveyDate][$v->jobNo][] = $tmp_arr;
}
