...
$t->set_file("HdIndex", "main_schedule_week_hki.html");
...
$msa->order = " ORDER BY plannedSurveyDate ASC,jobNo ASC,surveyType ASC,surveyLocation ASC,routeItems ASC";
...
for ($i = 1; $i <= $maxWeekNo; $i++) {
    $ms->weekNo = $i;
    // Tab设置
    $t->set_var(array("tabNo" => $i, "districtName" => '', "tabFullNo" => sprintf("%02d", $i)));
    ...
    $t->set_var(array("createDate" => $createDate, "updateDate" => $updateDate, "currYear" => $currYear, "weekDateStart" => $weekDateStart, "weekDateEnd" => $weekDateEnd, "weekNo" => $i, "tdNo" => $tdNo));
    ...
    for ($j = 0; $j < $rsNo; $j++) {
        $isPrint = false;
        if ($k % 2 == 0) $listStyle = "AlternatingItemStyle"; else
        ...
        if ($j == 0) {
            ...
            if (trim($mse->surveyType) == trim($msel->surveyType)
                ...
                $t->set_var(array("listStyle" => $listStyle, "plannedSurveyDate" => $mstmp->plannedSurveyDate, "jobNo" => $mstmp->jobNo, "tdFileNo" => $mstmp->tdFileNo, "receivedDate" => $mstmp->receivedDate, "dueDate" => $mstmp->dueDate, "fromTD" => $mstmp->fromTD, "surveyTimeHours" => $mstmp->surveyTimeHours, "surveyLocation" => $mstmp->surveyLocation, "routeItems" => $mstmp->routeItems, "direction" => $mstmp->direction, "noOfSurveyors" => $mstmp->noOfSurveyors, "estimatedManHour" => $mstmp->estimatedManHour, "onBoardCostHour" => $mstmp->onBoardCostHour));
                ...
                if ($tempPlannedSurveyDate >= $startShowDate && $tempPlannedSurveyDate < $endShowDate) {
                    $t->set_var(array("surveyorName" => $mstmp->surveyorName, "surveyorTelephone" => $mstmp->surveyorTelephone));
                } else {
                    $t->set_var(array("surveyorName" => "", "surveyorTelephone" => ""));
                }
    ...
    if (($i - 1) % 2 == 0) $listStyleSummary = "AlternatingItemStyle"; else
    ...
    $t->set_var(array("listStyleSummary" => $listStyleSummary, "weekNoSummary" => $weekNoSummary, "districtName" => $conf ['districtName'] [$complateJobNo], "estimatedManHourSummary" => $estimatedManHourSummary, "accuSummary" => $accuSummary, "onBoardCostSummary" => $onBoardCostSummary, "remainingHour" => $remainingHour));
